...
| Unified conversational UI validation | Available locally | 2026-05-17 validation commands | `scripts/test.ps1`, `scripts/test-eclipse-project-build.ps1`, `scripts/test-eclipse.ps1`, `scripts/test-eclipse.ps1 -UseBundleEnv` and `scripts/smoke-openai.ps1` passed after redesigning the Eclipse view as a conversational transcript. The runtime smoke test also verifies the new UI structure. |
| Single-transcript inline-code validation | Available locally | 2026-05-17 validation commands | `scripts/test.ps1`, `scripts/test-eclipse-project-build.ps1`, `scripts/test-eclipse.ps1`, `scripts/test-eclipse.ps1 -UseBundleEnv` and `scripts/smoke-openai.ps1` passed after moving all chat content into one read-only transcript with inline ABAP code blocks. |
...
| Single-transcript chat view screenshot | TODO/TBC | To be captured manually | Capture the updated view showing header, one scrollable transcript, bottom composer and inline ABAP code block using anonymised or non-confidential ABAP code. |
| Inline ABAP code review evidence | TODO/TBC | Manual Eclipse screenshot/test | Capture an assistant response containing a fenced ABAP suggestion; confirm inline `ABAP code` rendering and copy-only behaviour. |
