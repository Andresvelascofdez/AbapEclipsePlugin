...
| Conversational chat view screenshot | TODO/TBC | To be captured manually | Capture the updated view showing header, transcript, bottom composer and integrated suggested-change section using anonymised or non-confidential ABAP code. |
| Suggested change review evidence | TODO/TBC | Manual Eclipse screenshot/test | Capture an assistant response containing a fenced ABAP suggestion; confirm integrated `Suggested change` section and copy-only behaviour. |
